...
exports.getBlockAlignedStep = getBlockAlignedStep;
...
function blockSpan(doc, from, to) {
 ...
 */
function alignBlocks(fromBlocks, toBlocks) {
    ...
    for (let i = m - 1; i >= 0; i--) {
        for (let j = n - 1; j >= 0; j--) {
            ...
            if (eq[i][j]) {
                best = Math.max(best, fromBlocks[i].nodeSize + dp[i + 1][j + 1]);
            }
            dp[i][j] = best;
        }
    }
 ...
 */
function getBlockAlignedStep(fromDoc, toDoc) {
    const region = (0, getReplaceStep_1.findReplaceRegion)(fromDoc, toDoc, 0, fromDoc.content.size, 0, toDoc.content.size);
    if (region === null) {
        return false;
    }
    ...
    if (fromSpan.end - fromSpan.first <= 1 && toSpan.end - toSpan.first <= 1) {
        return (0, getReplaceStep_1.replaceStepForRegion)(toDoc, region);
    }
    ...
    if (pairs.length === 0) {
        if (fromBlocks.length === toBlocks.length) {
            ...
            const step = firstPairStep(fromDoc, toDoc, fromOffsets, toOffsets, fromSpan.first, toSpan.first, fromBlocks, toBlocks, 0, 0, fromBlocks.length);
            if (step) {
                return step;
            }
        }
    ...
    for (const gap of gaps) {
        ...
        if (m === 0) {
            const inserted = prosemirror_model_1.Fragment.from(toBlocks.slice(gap.t0, gap.t1));
            return new prosemirror_transform_1.ReplaceStep(fPos0, fPos0, new prosemirror_model_1.Slice(inserted, 0, 0));
        }
        if (n === 0) {
            return new prosemirror_transform_1.ReplaceStep(fPos0, fPos1, prosemirror_model_1.Slice.empty);
        }
        if (m === n) {
            const step = firstPairStep(fromDoc, toDoc, fromOffsets, toOffsets, fromSpan.first, toSpan.first, fromBlocks, toBlocks, gap.f0, gap.t0, m);
            if (step) {
                return step;
            }
            continue;
        }
        const step = (0, getReplaceStep_1.trimmedReplaceStep)(fromDoc, toDoc, fPos0, fPos1, tPos0, tPos1);
        if (step) {
            return step;
        }
    }
 ...
 */
function firstPairStep(fromDoc, toDoc, fromOffsets, toOffsets, fromFirst, toFirst, fromBlocks, toBlocks, f0, t0, count) {
    for (let k = 0; k < count; k++) {
        const step = (0, getReplaceStep_1.trimmedReplaceStep)(fromDoc, toDoc, fromOffsets[fromFirst + f0 + k], fromOffsets[fromFirst + f0 + k] + fromBlocks[f0 + k].nodeSize, toOffsets[toFirst + t0 + k], toOffsets[toFirst + t0 + k] + toBlocks[t0 + k].nodeSize);
        if (step) {
            return step;
        }
    }
...
//# sourceMappingURL=blockAlign.js.map