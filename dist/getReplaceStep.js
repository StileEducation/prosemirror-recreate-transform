 ...
 */
function calculateBoundaryScore(fromDoc, toDoc, start, endA, endB) {
    let score = 0;
...
function getReplaceStep(fromDoc, toDoc) {
    ...
    if (overlap > 0) {
        ...
        const scoreStart = calculateBoundaryScore(fromDoc, toDoc, start - overlap, endA, endB);
        const scoreEnd = calculateBoundaryScore(fromDoc, toDoc, start, endA + overlap, endB + overlap);
    ...
    return new prosemirror_transform_1.ReplaceStep(start, endB, toDoc.slice(start, endA));
}
